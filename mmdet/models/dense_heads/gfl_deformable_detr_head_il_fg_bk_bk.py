...
class GFLDeformableDETRHead_il(DETRHead):
    ...
    def loss(self,
        ...
        if self.has_teacher:
            ...
            if 'fg_info' in self.feats_distill and 'bg_info' in self.feats_distill:
                memory, spatial_shapes = info_all
                batch_pred_memory = memory.permute(1, 2, 0)
                batch_soft_memory = teacher_info['head_outs'][2][0].permute(1,2,0)
                N, C, HWall = batch_pred_memory.shape
                ...
                Mask_fg = [torch.zeros((N, hw[0],hw[1])).to(batch_pred_memory.device) for hw in spatial_shapes]
                Mask_bg = [torch.ones((N, hw[0],hw[1])).to(batch_pred_memory.device) for hw in spatial_shapes]
                # Mask_out = [torch.zeros((N, hw[0], hw[1])).to(batch_pred_memory.device) for hw in spatial_shapes]
                wmin, wmax, hmin, hmax = [], [], [], []

                sp_shape = spatial_shapes.shape[0]
                for i in range(N):
                    for sp in range(sp_shape):
                        new_boxxes = torch.ones_like(gt_bboxes_list[i])
                        new_boxxes[:, 0] = gt_bboxes_list[i][:, 0] / img_metas[i]['img_shape'][0] * spatial_shapes[sp][0]
                        new_boxxes[:, 2] = gt_bboxes_list[i][:, 2] / img_metas[i]['img_shape'][0] * spatial_shapes[sp][0]
                        new_boxxes[:, 1] = gt_bboxes_list[i][:, 1] / img_metas[i]['img_shape'][1] * spatial_shapes[sp][1]
                        new_boxxes[:, 3] = gt_bboxes_list[i][:, 3] / img_metas[i]['img_shape'][1] * spatial_shapes[sp][1]
                        ...
                        # for j in range(len(teacher_info['pred_bboxes'][i]), len(gt_bboxes_list[i])):
                        #     Mask_out[sp][i][hmin[j]:hmax[j] + 1, wmin[j]:wmax[j] + 1] = \
                        #         torch.maximum(Mask_fg[sp][i][hmin[j]:hmax[j] + 1, wmin[j]:wmax[j] + 1], area[0][j])

                        # Mask_fg[sp][i] = torch.where((Mask_fg[sp][i]  + Mask_out[sp][i]) > 0, 1, 0)
                        Mask_bg[sp][i] = torch.where((Mask_fg[sp][i]) > 0, 0, 1)
                        if torch.sum(Mask_bg[sp][i]):
                            Mask_bg[sp][i] /= torch.sum(Mask_bg[sp][i])
                ...
                bg_loss = [self.bg_feature_calculation(s_memory, t_memory, m_bg)
                                    for s_memory, t_memory, m_bg in zip(batch_pred_memory, batch_soft_memory, Mask_bg_all)]
                avg_factor = [1, len(bg_loss), len(img_metas), len(teacher_info['pred_keepid']), memory.shape[2]*len(img_metas)][2]
                loss_bg_feature = sum(bg_loss)/avg_factor
                loss_dict.update({'loss_bg_feature': loss_bg_feature})
        return loss_dict
