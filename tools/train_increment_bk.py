...
from mmdet.datasets import build_dataset, build_dataloader, replace_ImageToTensor, split_data_category
...
def parse_args():
    ...
    parser.add_argument('--work-dir',
                        ...
                        default='/home/softlink/zhjpexp/yolof-r18-stst-qoqo-v3',
...
def main():
    ...
    # 任务设定
    catsplit = cfg.task.catsplit
    

    trainsplit, valsplit, finesplit = \
        split_data_category(dataname=cfg.data.train.type, split=cfg.task.catsplit, order=cfg.task.catorder,
                            catofset=cfg.task.catofset, valpart=cfg.task.valpart)
    task_list = [f'Task{i+1}' for i, n in enumerate(cfg.task.catsplit)]
    task_index = list(range(1, len(cfg.task.catsplit) + 1))
    task_catenums = [len(valsplit[tid - 1]) for tid in task_index]
    # 每次任务的微调学习率和权值衰减率，根据当前任务需要优化的参数组进行设定
    ...
    for tid, task in zip(task_index, task_list):
        ...
        if tid == 2:
            break
        # 数据加载
        traincats, valcats, finecats = trainsplit[tid - 1], valsplit[tid - 1], finesplit[tid - 1]
        traincats, valcats, finecats = list(traincats.keys()), list(valcats.keys()), list(finecats.keys())
        cfg.data.train.update({'classes': traincats}) or cfg.data.val.update({'classes': valcats})
        cfg.data.train.update({'learn_mode': 'increment'}) or cfg.data.val.update({'learn_mode': 'increment'})
        train_dataset = build_dataset(cfg.data.train)
        val_dataset = build_dataset(cfg.data.val, dict(test_mode=True))
        train_dataloader = build_dataloader(
            train_dataset, cfg.data.samples_per_gpu, cfg.data.workers_per_gpu,
            num_gpus=len(cfg.gpu_ids), dist=distributed, seed=cfg.seed, runner_type=cfg.runner[tid - 1]['type'],
            persistent_workers=cfg.data.get('persistent_workers', False))
        val_dataloader = build_dataloader(
            val_dataset, cfg.data.val.pop('samples_per_gpu', 1), cfg.data.workers_per_gpu,
            dist=distributed, shuffle=False)

        print(f'\n图像数量 for Train: {len(train_dataset)} & Val: {len(val_dataset)}')
        print(f'训练集加载类别==> {len(train_dataset.CUR_CLASSES)}', train_dataset.CUR_CLASSES)
        print(f'验证集加载类别==> {len(val_dataset.CUR_CLASSES)}', val_dataset.CUR_CLASSES)
        ...
        #     gt_labels_batch = batch_data['gt_labels'].data[0]
        #     gt_labels_batch = [l.numpy().tolist() for l in gt_labels_batch]
        #     gt_bboxes_batch = batch_data['gt_bboxes'].data[0]
        #     gt_bboxes_batch = [l.numpy().tolist() for l in gt_bboxes_batch]
        ...
        # 模型加载
        if tid == 0 or 1:
            # cfg.model['bbox_head']['num_classes'] = len(train_dataset.CLASSES)
            ...
            model.set_teacher(model=copy.deepcopy(model), trainval='val')
            model.set_student()
        ...
        if distributed:
            ...
            # model.module.teacher_model.train(False)
