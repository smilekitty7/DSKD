...
class DeformableDETR_il(SingleStageDetector):
    def __init__(self,
                 backbone,
                 neck,
                 bbox_head,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(DeformableDETR_il, self).__init__(backbone, neck, bbox_head, #teacher_config, teacher_ckpt, eval_teacher, teacher_test_cfg,
    ...
    def out_teacher(self, img, img_metas, cat_keepid=True):
        ...
        with torch.no_grad():
            ...
            head_outs = self.teacher_model.bbox_head.forward(neck_feat, img_metas)
            ...
            if cat_keepid:
                ...
                pred_keepid = torch.cat([pk + i * head_outs[0].shape[2] for i, pk in enumerate(pred_keepid)])
