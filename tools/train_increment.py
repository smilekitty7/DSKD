...
def main():
    ...
    for tid, task in zip(task_index, task_list):
        ...
        if tid == 1:
            ...
            model.set_teacher(config=teacher_config, ckptfile=teacher_ckpt, model=None, trainval='val')
            if tid == cfg.task.resume_by_task:
            ...
            if tid == cfg.task.resume_by_task:
                ...
                model.set_teacher(config=teacher_config, ckptfile=teacher_ckpt, trainval='val',
                                  model=copy.deepcopy(model) if teacher_ckpt is None else None)
        ...
        if cfg.task.resume_by_epoch:
            print(f'\n在任务 {task} 的 XX回合上加载学生模型！cfg.task.resume_by_epoch=True')
...
#             print(f'\n\n======== Task-{tid}({task}) 跳过, At {datetime.datetime.now()} ==========')
#             continue
#
#         # 模型加载
#         if tid == 1:
...
#             model.set_teacher(config=teacher_config, ckptfile=teacher_ckpt, model=None, trainval='val')
#             if tid == cfg.task.resume:
#                 print(f'\n====== Task-{tid}({task}) 跳过，Resume From CheckPoint: {student_ckpt}')
#                 continue
#         else:
#             if tid == cfg.task.resume:
#                 model = build_detector(cfg.model, train_cfg=cfg.get('train_cfg'), test_cfg=cfg.get('test_cfg'))
...
#                 if isinstance(model, (MMDataParallel, MMDistributedDataParallel)):
#                     model = model.module
#                 # 可手动为某次任务配置教师模型，否则自动使用上次任务模型作为下次任务的教师。
#                 teacher_config, teacher_ckpt = None, None
#                 if getattr(cfg.task, task, None) and getattr(cfg.task[task], 'load_teacher', False):
#                     teacher_config = cfg.task[task]['teacher_config']
#                     teacher_ckpt = cfg.task[task]['teacher_ckpt']
